...
export function auto(ns: NS, ramOverride?: number) {
    const reservation = reserve(ramOverride ?? ns.getScriptRam(ns.getScriptName()));
    if (!reservation) throw new Error("Failed to reserve memory for automatic process management.");
