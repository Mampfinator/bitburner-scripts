...
async function connectToRemoteApi(): Promise<boolean> {
    ...
    const settingsButton: HTMLDivElement | null = doc.querySelector("div[role='button']:has(svg[aria-label='Options'])");
    ...
    const settingsPage = doc.querySelector("div.MuiContainer-root.MuiContainer-maxWidthLg.MuiContainer-disableGutters.css-5sgl1v");
    console.log(settingsPage);
    if (!settingsPage) return false;

    const settingsTabs = settingsPage.querySelectorAll("div.MuiButtonBase-root.MuiListItemButton-root.MuiListItemButton-gutters.MuiListItemButton-root.MuiListItemButton-gutters.css-1593q0[role='button']");
    ...
    const remoteApiMenu: HTMLDivElement | null = settingsPage.querySelector("div.MuiPaper-root.MuiPaper-elevation.MuiPaper-rounded.MuiPaper-elevation1.css-d7p367");
    if (!remoteApiMenu) return false;

    const connectButton: HTMLButtonElement | null = remoteApiMenu.querySelector("button.MuiButtonBase-root.MuiButton-root.MuiButton-text.css-gew9km[type='button']");
...
export async function main(ns: NS) {
    ...
    if (!success) {
        ns.alert("All files were deleted, but connection to the remote API for reacquiring files failed. Please connect manually, save and reload the game.");
    }
