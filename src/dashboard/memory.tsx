...
export const MemoryInspector = ({ns}: {ns: NS}) => {
    ...
    const [memInfo, setMemInfo] = React.useState<Map<string, {memInfo: MemInfo, actual: number}>>(() => calculateMem());
    ...
    return <div>
        <details>
            ...
            <div>
                {[...memInfo].filter(([_, {memInfo, actual}]) => actual > 0 && memInfo.available < memInfo.capacity).map(([server, {memInfo, actual}]) => <ServerMemInfo server={server} memInfo={memInfo} actual={actual}/>)}
            </div>
        </details>
    </div>
}
...
const ServerMemInfo = (props: ServerMemInfoProps) => {
    const {actual, server, memInfo: { capacity, available }} = props;
    return <div style={{display: "flex", flexDirection: "column", border: "1px solid darkgreen"}}>
...
const UsageBar = ({label, max, valueA, valueB}: UsageBarProps) => {
    ...
    return <div>
        ...
        <div style={CONTAINER_STYLE}>
            <div style={{...BAR_STYLE, width: `${widthA * 100}%`, backgroundColor: "blue"}}/>
            {widthB > 0 && <div style={{...BAR_STYLE, width: `${widthB * 100}%`, backgroundColor: "red"}}/>}
        </div>
    </div>
}