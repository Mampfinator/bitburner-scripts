        ...
        try {
            ...
            for await (const result of ctx.run({ filter })) {
                if (!result.success) {
                    ...
                    ns.tprint(`${" ".repeat(4)}${col().red("x")} ${name} (${time}ms): ${col().red(error.message)}`);
                ...
                if (result.type === "timing") {
                    ...
                    ns.tprint(`${" ".repeat(4)}${col().cyan("✓")} ${name} (${min}ms min./${avg}ms avg./${max}ms max.)`);
                } else if (result.type === "default") {
                    const { name, time } = result;
                    ns.tprint(`${" ".repeat(4)}${col().cyan("✓")} ${name} (${time}ms)`);
                }
            }
        } catch (e) {
            ns.tprint(
                col().red(`Test ${col().yellow(scriptName)} threw an unknown error. Check the console for details.`),
            );
            console.error(e);
