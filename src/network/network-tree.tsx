...
function NetworkTree({ns}: {ns: NS}): React.ReactElement {
    const serverGraph = getServerGraph(ns, {startFrom: "home", backdoorIsHomeLink: false});
    ...
    pushNode("home", WIDTH / 2, SPACE_Y * 3, [["source", PositionEnum.Left], ["source", PositionEnum.Right]]);
    ...
    for (const server of [...tree.children.values()]
        ...
        .sort((a, b) => Number(a.replace("home", "")) - Number(b.replace("home", "")))
    ) {
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        if (i === 0) {
            handles = [["target", PositionEnum.Left], ["source", PositionEnum.Right]];
        } else if (i === rows.length - 1) {
            handles = [["target", PositionEnum.Left]];
        } else {
            handles = [["source", PositionEnum.Right], ["target", PositionEnum.Left]];
        }
