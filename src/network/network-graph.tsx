...
const { 
    d3: { forceSimulation, forceManyBody, forceLink, forceX, forceY, forceCollide: collide },
    React, React: { useMemo },
    ReactFlow: { ReactFlow, ReactFlowProvider, Panel, useNodesState, useEdgesState, useReactFlow, useNodesInitialized }, 
} = globalThis;
...
const LayoutFlow = ({ns}: {ns: NS}) => {
  ...
  const initialNodes = [...graph.nodes].map(serverName => {
    return {
      id: serverName,
  ...
  for (const [from, tos] of graph.edges) {
    ...
    for (const to of tos) {
      ...
      if (!edgeMap.has(a)) edgeMap.set(`${a}-${b}`, { source: a, target: b, id: `${a}-${b}` });
    }
  }
  ...
  const [initialized, { toggle, isRunning }] = useLayoutedElements() as [boolean, { toggle: () => void, isRunning: () => boolean }];
