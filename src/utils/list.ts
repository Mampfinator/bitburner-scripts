...
function calculateBatchRatios(ns: NS, hostname: string, hackRatio: number = 0.35) {
    ...
    const growThreads = Math.ceil(ns.growthAnalyze(server.hostname, 1 / (1 - hackRatio)));
...
export async function main(ns: NS) {
    ...
    if (mode === "owned") {
        ...
        for (const server of servers) {
            ...
            const skillPrefix =
                (server.requiredHackingSkill ?? 0) > ns.getPlayer().skills.hacking ? `\x1b[31m` : `\x1b[36m`;

            const portsString = `${server.numOpenPortsRequired}\x1b[0m`;
            const portsPrefix = (server.numOpenPortsRequired ?? 0) > numPortCrackers ? `\x1b[31m` : `\x1b[36m`;
        ...
        for (const server of servers) {
            ...
            const skillPrefix =
                (server.requiredHackingSkill ?? 0) > ns.getPlayer().skills.hacking ? `\x1b[31m` : `\x1b[36m`;
            ns.tprint(`\x1b[1m${server.hostname}\x1b[0m: ${skillPrefix}${skillString}`);
        }
